// https://github.com/LeekJay/unocss-preset-component
...
import type { CSSEntries, Preflight, Preset, Rule, Shortcut } from 'unocss'
...
function parseStyles(styles: Arrayable<Style>, postcssPlugins: Arrayable<AcceptedPlugin> = []) {
  return toArray(styles).flatMap(style =>
    isPlainObject(style) && !['root', 'document'].includes((style as Root).type)
      // css in js object
      ? postcss(toArray(postcssPlugins)).process(style as CssInJs, { parser: cssInJsParser }).root.nodes as ChildNode[]
